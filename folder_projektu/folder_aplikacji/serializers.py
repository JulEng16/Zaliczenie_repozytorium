...
class StanowiskoSerializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        return Stanowisko.objects.create(**validated_data)   #** - że będzie dużo obiektów. * - jakas liczba, ilosc blizej nieokreslona ** - slownik
...
class OsobaSerializer(serializers.ModelSerializer):
    ...
    def validate_data_dodania(self, value):
        if value > date.today():
             raise serializers.ValidationError("Pole 'data' nie może być z przyszłości!")
        return value
    class Meta: 
        model = Osoba
